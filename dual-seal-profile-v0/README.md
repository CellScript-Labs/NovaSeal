...
`cellscript-tools profile-operator-fixtures`. It binds the finalisation
...
`cellscript-tools service-builder-fixtures` generates the matching service
request/response skeleton and preserves the same public BTC evidence boundary.
`cellscript-tools btc-spv-evidence-adapter` generates the external BTC SPV
