...
| `cargo run --quiet --locked --manifest-path ../tools/Cargo.toml -- agreement-tx-shape --pretty` | passed; 12/12 shape and arithmetic-boundary expectations matched |
...
| `cellscript-tools novaseal-agreement-devnet --pretty --ckb-repo ../ckb --ckb-bin ../ckb/target/debug/ckb` | passed; originate -> repay, originate -> claim, and live negative dry-runs |
| `cargo run --quiet --locked --manifest-path ../../../Cargo.toml -p cellscript-tools --bin cellscript-tools -- wallet-signing-vectors --pretty` | passed; includes 3 Agreement vectors |
...
cargo run --quiet --locked --manifest-path ../../../Cargo.toml -p cellscript-tools --bin cellscript-tools -- --root ../../.. novaseal-agreement-devnet --pretty --ckb-repo /path/to/ckb --ckb-bin /path/to/ckb/target/debug/ckb
...
`crates/cellscript-tools/src/wallet_vectors.rs` and checked by the compiler-hosted
