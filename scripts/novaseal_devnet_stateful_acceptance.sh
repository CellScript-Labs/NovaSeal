...
summary="$(cargo run --quiet --locked --manifest-path "$ROOT_DIR/../../Cargo.toml" \
  -p cellscript-tools --bin cellscript-tools -- --root "$ROOT_DIR/../.." \
  novaseal-acceptance-summary "$REPORT")"
